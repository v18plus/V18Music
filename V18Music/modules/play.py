...
from V18Music.config import ARQ_API_KEY
from V18Music.config import BOT_NAME as bn
from V18Music.config import DURATION_LIMIT
from V18Music.config import ASSISTANT_NAME
from V18Music.config import UPDATES_CHANNEL as updateschannel
from V18Music.config import que
from V18Music.function.admins import admins as a
from V18Music.helpers.admins import get_administrators
from V18Music.helpers.channelmusic import get_chat_id
from V18Music.helpers.errors import DurationLimitError
from V18Music.helpers.decorators import errors
from V18Music.helpers.decorators import authorized_users_only
from V18Music.helpers.filters import command
from V18Music.helpers.filters import other_filters
from V18Music.helpers.gets import get_file_name
from V18Music.services.callsmusic import callsmusic
from V18Music.services.callsmusic import client as USER
from V18Music.services.converter.converter import convert
from V18Music.services.downloaders import youtube
from V18Music.services.queues import queues
...
async def play(_, message: Message):
    ...
    if message.chat.id in DISABLED_GROUPS:
        return
    lel = await message.reply("🔄 **Grace Sedang Memproses Lagu**")
    ...
    try:
        ...
        return
    text_links=None
    await lel.edit("🔄 **@graclex Sedang Mencari Lagu**")
    ...
    if audio:
        ...
        keyboard = InlineKeyboardMarkup(
            [
                [
                    ...
                    InlineKeyboardButton("⛑ Channel", url="https://t.me/viraltiktokgroups"),
                ],
        ...
        query = toxt
        await lel.edit("🎵 **Grace Sedang Memproses Lagu**")
...
async def deezer(client: Client, message_: Message):
    if message_.chat.id in DISABLED_GROUPS:
        return
    global que
    lel = await message_.reply("🔄 **Grace Sedang Memproses Lagu**")
    ...
    query = queryy
    res = lel
    await res.edit(f"**@graclex Sedang Mencari Lagu** `{query}` **dari deezer**")
    try:
...
async def lol_cb(b, cb):
    ...
    if cb.from_user.id != useer_id:
        ...
        return
    await cb.message.edit("🔄 **Grace Sedang Memproses Lagu**")
    x=int(x)
    try:
